...
const reducer = (state, action) => {
  switch (action.type) {
    ...
    case 'SAVE_REVIEWS_SUCCESS':
      return { ...state, saveLoading: false, product: action.payload };
...
export default function ProductDetails() {
  ...
  const [{ loading, error, product, reviewError, reviewLoading, saveError, saveLoading }, dispatch] =
    useReducer(reducer, { loading: true, error: "", product: [], saveError: '', saveLoading: false, reviewLoading: false, reviewError: '' });
        ...
        (
          <div>
            ...
            <Grid container item spacing={2}>
              ...
              <Grid item md={8} sm={10} xs={12}>
                <h2>Reviews</h2>
                {reviewLoading ? <CircularProgress /> : reviewError ? <Alert severity='error' >{reviewError}</Alert> : (null)}
                <List>
                  {
