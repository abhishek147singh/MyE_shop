...
productRouter.post("/review", isAuth , expressAsyncHandler(async (req, res) => {
    
    try{
        const prevReview = await Product.find({ "_id" :req.body.product , "reviews.user" : req.user._id });
        
        if(prevReview.length > 0){
    
            await Product.updateOne({ "_id": req.body.product , "reviews": { "$elemMatch" : { "user" : req.user._id } } } ,
                                    { $set : { "reviews.$.message" : req.body.comment , "reviews.$.rating" : req.body.rating } });
            
            let product = await Product.findById(req.body.product);
             
            const totalRating = product.reviews.reduce(( acc , current ) => {
                return acc + current.rating; 
            }, 0);

            const avgRating = totalRating  / product.noReviews;
            ...
            product.noReviews = product.noReviews + 1;
